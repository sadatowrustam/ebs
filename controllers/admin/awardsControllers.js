...
exports.getAllBlogs=catchAsync(async(req,res,next)=>{
    ...
    const count=await Awards.count({where})
    return res.send({data,count})
})
exports.addBlogs=catchAsync(async(req,res,next)=>{
    const blogs=await Awards.create(req.body);
    return res.status(201).send(blogs)
})
exports.editBlogs=catchAsync(async(req,res,next)=>{
    const awards=await Awards.findOne({where:{id:req.params.id}})
    if(!awards) return next(new AppError("Awards not found",404))
    console.log(req.body)
    await awards.update(req.body)
...
exports.deleteBlogs=catchAsync(async(req,res,next)=>{
    ...
    if(!blogs) return next(new AppError("Awards not found",404))
    await blogs.destroy()
