...
exports.getAllBlogs=catchAsync(async(req,res,next)=>{
    ...
    const data=await Trusting.findAll({
        where,
    ...
    const count=await Trusting.count({where})
    return res.send({data,count})
})
exports.addBlogs=catchAsync(async(req,res,next)=>{
    console.log(req.body)
    const blogs=await Trusting.create(req.body);
    await Images.update({trustingId:blogs.id},{where:{id:{[Op.in]:req.body.images}}})
    return res.status(201).send(blogs)
})
exports.editBlogs=catchAsync(async(req,res,next)=>{
    const blogs=await Trusting.findOne({where:{id:req.params.id}})
